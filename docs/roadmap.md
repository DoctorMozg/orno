...
- **Whole-node plugin protocol** (ADRs 0004, 0017; remains deferred. ADR 0017 removes `NodeKind::External` from v0.1 entirely and supersedes ADR 0014's `--unstable-plugins` gate. Reintroduction at v0.2+ takes a `transport: builtin | subprocess` axis on existing kinds, not a sibling kind.)
- **`gate` node** (ADR 0017) — the 2/5 dissent in the 2026-04-21 node-taxonomy brainstorm. Kind-vs-attribute (`when:` / `await:`) unsettled; revisit when a real user asks for conditional branching or approval-hold.
- **`checkpoint` node** (ADR 0017) — content-addressed cache/resume. Deferred until the event schema stabilizes (ADRs 0003/0012) and a real pipeline validates the cache-key invariant. Revisit on first reproducible mid-DAG resume or re-run cache request.
- **`probe` node** (ADR 0017) — pre-flight assertion wrapper. Deferred until a real user reports wasted LLM spend on a precondition that could have been asserted up front.
- **`emit_metrics` node** (ADR 0017) — Prometheus/OTLP aggregates at DAG completion. Requires `needs: [*]` terminal-node scheduling pattern. Revisit when first production adopter asks.
- **`pure` node** (ADR 0017) — deterministic transform kind. MiniJinja in `vars` and agent prompts covers this at v0.1. Revisit when a user wants a transform MiniJinja cannot express without agent scaffolding.
